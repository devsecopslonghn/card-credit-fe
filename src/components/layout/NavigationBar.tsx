    ...
    <>
      <aside className={`cc-sidebar fixed inset-y-0 left-0 z-40 hidden w-64 flex-col border-r lg:flex ${adminMode ? "border-slate-700 bg-slate-900" : "border-gray-200 bg-white"}`} aria-label={adminMode ? "Điều hướng quản trị" : "Điều hướng chính"}>
        <div className="flex items-center gap-3 p-6">
          ...
          <Link to={adminMode ? "/admin/users" : "/dashboard"} className={`text-xl font-bold tracking-tight ${adminMode ? "text-white" : "text-gray-900"}`}>Personal Finance</Link>
        </div>
        <nav className="flex-1 space-y-1 px-4 py-4">
          {adminMode ? <Link to="/cards" className="mb-4 flex items-center gap-3 rounded-xl border border-slate-700 px-3 py-2.5 text-sm font-semibold text-slate-200 hover:bg-slate-800">← Quay lại User Dashboard</Link> : null}
          {!adminMode && isAdmin ? <Link to="/admin/users" className="mb-4 flex items-center justify-between rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-sm font-bold text-amber-800 hover:bg-amber-100"><span>Admin Console</span><span aria-hidden="true">→</span></Link> : null}
          {menuLinks.map((link) => <Link key={`${link.href}-${link.label}`} to={link.href} aria-current={active(link.href) ? "page" : undefined} className={navLink(link.href)}><span className="w-5 text-center text-lg leading-none" aria-hidden="true">{linkIcon(link)}</span>{link.label}</Link>)}
        </nav>
        <div className={`flex items-center justify-between border-t p-4 ${adminMode ? "border-slate-700" : "border-gray-100"}`}>
          <Link to="/profile" className={`min-w-0 truncate text-sm font-semibold ${adminMode ? "text-slate-200" : "text-gray-700"}`}>{user.displayName || user.email}</Link>
      ...
      <header className={`cc-mobile-top sticky top-0 z-40 flex items-center justify-between border-b px-4 py-3 shadow-sm backdrop-blur lg:hidden ${adminMode ? "border-slate-700 bg-slate-900 text-white" : "border-gray-200 bg-white/95"}`}>
        <Link to={adminMode ? "/admin/users" : "/dashboard"} className={`flex items-center gap-2 text-lg font-bold tracking-tight ${adminMode ? "text-white" : "text-gray-900"}`}><span className={`flex h-7 w-7 items-center justify-center rounded-lg text-sm text-white ${adminMode ? "bg-slate-700" : "bg-blue-700"}`}>₫</span>{adminMode ? "Admin Console" : "Personal Finance"}</Link>
        <Link to="/profile" aria-label="Mở hồ sơ" className={`flex h-9 w-9 items-center justify-center rounded-full text-sm font-bold ${adminMode ? "bg-slate-700 text-white" : "bg-blue-50 text-blue-700"}`}>{(user.displayName || user.email).slice(0, 1).toUpperCase()}</Link>
      </header>
