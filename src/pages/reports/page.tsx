...
export default function ReportsPage() {
  ...
  return <FinanceShell title="Báo cáo tài chính">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5"><Metric label="Personal spending" value={data?.totals.personalSpending ?? 0}/><Metric label="Debit/Cash/E-wallet flow" value={data?.totals.debitCashflow ?? 0} tone="positive"/><Metric label="Nợ phát sinh trong kỳ" value={data?.totals.creditDebt ?? 0} tone="debt"/><Metric label="Dư nợ hiện tại" value={data?.totals.currentCardDebt ?? 0} tone="debt"/><Metric label="Khoản phải thu" value={data?.totals.outstandingReceivable ?? 0} tone="receivable"/></div>
    <section className="cc-section mt-6 p-5"><h2 className="text-xl font-bold">Dư nợ hiện tại theo thẻ</h2><p className="mt-1 text-sm cc-text-muted">Bao gồm số dư sao kê chưa thanh toán và điều chỉnh đối soát kỹ thuật.</p><div className="mt-4 space-y-3">{(data?.currentDebtLedger ?? []).filter((item) => item.currentDebt > 0).map((item) => <div key={item.cardId} className="flex flex-col gap-2 rounded-lg border p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">{item.providerName} · {item.displayName}</p><p className="text-sm cc-text-muted">Sao kê còn nợ {vnd(item.statementOutstanding)} · Điều chỉnh {vnd(item.technicalAdjustment)}{item.nextPaymentDue ? ` · Hạn gần nhất ${item.nextPaymentDue}` : ""}</p></div><p className="text-xl font-bold cc-tabular">{vnd(item.currentDebt)}</p></div>)}{(data?.currentDebtLedger ?? []).every((item) => item.currentDebt <= 0) && <p className="text-sm cc-text-muted">Không có dư nợ hiện tại.</p>}</div></section>
